...
/**
 * Heals line endings and splits in raw JSON exports from HoneyMoney
 */
export function healRawHoneyJson(rawJson: string): string {
  // Split into lines
  ...
  for (const line of lines) {
    ...
    // Standard JSON structure lines in this export start with specific prefix templates
    const isStandard = 
      line.startsWith('        "') || 
      line.startsWith('    {') || 
      line.startsWith('    },') || 
  ...
  cleaned = cleaned.replace(/nu\s+ll/g, 'null');
  return cleaned;
}

/**
 * Converts a relaxed HoneyMoney records array or ready-made project object into standardized FinanceData
 */
export function parseAndStandardizeJsonToFinanceData(rawText: string): FinanceData {
  let cleanedText = rawText;
  
  // 1. Try safe healing first
  try {
    cleanedText = healRawHoneyJson(rawText);
  } catch (err) {
    console.warn("Failed healing phase, trying to parse raw string directly:", err);
  }

  const parsed = JSON.parse(cleanedText);

  // 2. Case A: Ready-made struct
  ...
  if (Array.isArray(parsed)) {
    ...
    parsed.forEach((item: any, idx: number) => {
      ...
      if (transferStr && transferStr.includes('=>')) {
        ...
        if (!realAccountsAllowlist.has(accountName)) return;
  ...
  throw new Error("Неверный формат JSON файла. Ожидался либо массив записей HoneyMoney, либо объект со структурой FinanceData.");
}
